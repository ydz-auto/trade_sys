#!/usr/bin/env python3
"""
Quick test version - run first 3 strategies only
"""
import sys
import os
...
class WalkForwardRunner:
    ...
    def generate_report(self) -> str:
        report = []
        report.append("\n" + "="*80)
        report.append("QUICK TEST - Walk-Forward Report (First 3 Strategies)")
        report.append("="*80)
        ...
        for result in self._results:
            ...
            report.append(f"  Overfitting: {result.overfitting_score:.2%}")
        
        report.append("\n" + "="*80)
        report.append("✅ Quick test completed successfully!")
        report.append("="*80)
        return "\n".join(report)

def main():
    print("="*80)
    print("QUICK TEST - First 3 Strategies (Single parameter each)")
    print("="*80)
    ...
    for strategy_id in ALL_STRATEGIES:
        try:
            ...
            logger.error(f"Failed for {strategy_id}: {e}")
            import traceback
            traceback.print_exc()
