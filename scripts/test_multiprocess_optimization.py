...
backend_path = os.path.abspath(os.path.join(scripts_path, 'backend'))
sys.path.insert(0, backend_path)

spec = importlib.util.spec_from_file_location("run_walkforward_fixed", os.path.join(scripts_path, "run_walkforward_fixed.py"))
run_wf_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(run_wf_module)
WalkForwardRunner = run_wf_module.WalkForwardRunner

def test_multiprocess_optimization():
    print("="*80)
    print("测试多进程参数优化")
    print("="*80)
...
if __name__ == "__main__":
    test_multiprocess_optimization()
