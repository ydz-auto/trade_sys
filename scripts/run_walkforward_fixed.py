...
class WalkForwardRunner:
    ...
    def _load_funding_data(self) -> Optional[pd.DataFrame]:
        data_path = Path(backend_path) / "data_lake" / "deribit" / "funding_rates" / "BTC-USDT-SWAP.parquet"
        if data_path.exists():
    ...
    def _load_oi_data(self) -> Optional[pd.DataFrame]:
        data_path = Path(backend_path) / "data_lake" / "deribit" / "open_interest" / "BTC-USDT-SWAP.parquet"
        if data_path.exists():
            df = read_parquet_safe(data_path)
            if df is not None and len(df) > 0:
                df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)
    ...
    def optimize_params(
        ...
        if use_multiprocess and len(param_combinations) > 1:
            logger.info(
                f"Using AccelerationService.parallel_map: {len(param_combinations)} combos"
            )
            ...
            logger.info(
                f"Using sequential optimization: {len(param_combinations)} combos"
            )
        ...
        for result in results:
            ...
            if sharpe > best_sharpe:
                ...
                best_return = result.get("return", 0.0)
    ...
    def split_walkforward_windows(
        ...
        start_date = datetime(year, 1, 1, tzinfo=pd.UTC)
        end_date = datetime(year + 1, 1, 1, tzinfo=pd.UTC)
        ...
        train_end = datetime(year, 6, 1, tzinfo=pd.UTC)
        
        val_start = train_end
        val_end = datetime(year, 9, 1, tzinfo=pd.UTC)
