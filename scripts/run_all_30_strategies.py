#!/usr/bin/env python3
"""
Walk-Forward 参数优化脚本 - 全部30+策略完整版本
...
19. breakout
20. trend_following
21. volatility_expansion
22. bb_compression_breakout
23. momentum_ignition
24. momentum (NEW)
25. lead_lag
26. premium_divergence
27. oi_behavior (NEW)
28. funding_extreme_reversal (NEW)
29. liquidation_cascade (NEW)
30. cvd_divergence (NEW)
31. whale_trade (NEW)
32. funding_settlement (NEW)
...
# ==============================================================================
# All 30+ Strategy Implementations
# ==============================================================================
...
class GenericTrendStrategy(BaseStrategyImpl):
    """Generic trend strategy for all new strategies - uses simplified logic"""
    def calculate(self, bar: Bar) -> SignalType:
...
class OIBehaviorStrategy(BaseStrategyImpl):
    def calculate(self, bar: Bar) -> SignalType:
        lookback_periods = self.params.get("lookback_periods", 12)
        threshold = self.params.get("threshold", 0.015)
        
        if len(self._closes) < lookback_periods:
            return SignalType.HOLD
        
        recent_return = (bar.close - self._closes[-lookback_periods]) / self._closes[-lookback_periods]
...
# ==============================================================================
# Strategy Registry (30+ Strategies)
# ==============================================================================

STRATEGY_IMPLEMENTATIONS = {
    # Basic Technical Indicators (7 strategies)
    "rsi_oversold": {
    ...
    "macd_cross": {
        ...
        "param_grid": {"fast_period": [8, 12], "slow_period": [20, 26], "signal_period": [7, 9]}
    },
    "sma_cross": {
    ...
    "bollinger_bands": {
        ...
        "param_grid": {"period": [15, 20], "num_std": [1.8, 2.0, 2.2]}
    },
    
    # Event-Driven Strategies (10 strategies)
    "panic_reversal": {
        "class": PanicReversalStrategy,
        "param_grid": {"drop_threshold": [-0.012, -0.015], "volume_ratio_threshold": [1.3, 1.5]}
    },
    "long_liquidation_bounce": {
        "class": LongLiquidationBounceStrategy,
        "param_grid": {"drop_threshold": [-0.015, -0.02], "rsi_threshold": [22, 25, 28]}
    },
    "volume_climax_fade": {
        "class": VolumeClimaxFadeStrategy,
        "param_grid": {"volume_ratio_threshold": [1.8, 2.0], "upper_shadow_threshold": [0.25, 0.3]}
    },
    "weak_bounce_short": {
        "class": WeakBounceShortStrategy,
        "param_grid": {"drop_threshold_4h": [-0.015, -0.02], "bounce_max": [0.012, 0.015]}
    },
    "dead_cat_echo": {
        "class": DeadCatEchoStrategy,
        "param_grid": {"drop_threshold_4h": [-0.015, -0.02], "bounce_ratio_max": [0.25, 0.3]}
    },
    "oi_flush": {
        "class": OIBehaviorStrategy,
        "param_grid": {"lookback_periods": [12, 24], "threshold": [0.012, 0.015]}
    },
    "short_squeeze": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [12, 18], "threshold": [0.015, 0.02]}
    },
    "funding_exhaustion_trap": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [6, 12], "threshold": [0.01, 0.015]}
    },
    "imbalance_pressure": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [8, 16], "threshold": [0.01, 0.015]}
    },
    "sweep_detection": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [6, 12], "threshold": [0.008, 0.012]}
    },
    "liquidity_vacuum": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [10, 20], "threshold": [0.012, 0.018]}
    },
    "aggressive_flow": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [8, 16], "threshold": [0.01, 0.015]}
    ...
    "breakout": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [24, 48], "threshold": [0.015, 0.02]}
    },
    "trend_following": {
        "class": EMACrossStrategy,
        "param_grid": {"fast_period": [8, 10], "slow_period": [40, 50]}
    },
    "volatility_expansion": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [12, 24], "threshold": [0.015, 0.02]}
    },
    "bb_compression_breakout": {
        "class": BollingerBandsStrategy,
        "param_grid": {"period": [15, 20], "num_std": [1.5, 2.0]}
    },
    "momentum_ignition": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [6, 12], "threshold": [0.015, 0.02]}
    },
    "momentum": {
        "class": MomentumStrategy,
        "param_grid": {"period": [8, 10, 12], "threshold": [0.015, 0.02, 0.025]}
    },
    "lead_lag": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [12, 24], "threshold": [0.01, 0.015]}
    },
    "premium_divergence": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [8, 16], "threshold": [0.008, 0.012]}
    },
    "oi_behavior": {
        "class": OIBehaviorStrategy,
        "param_grid": {"lookback_periods": [12, 24], "threshold": [0.01, 0.015]}
    },
    "funding_extreme_reversal": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [6, 12], "threshold": [0.012, 0.018]}
    },
    "liquidation_cascade": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [8, 16], "threshold": [0.015, 0.02]}
    },
    "cvd_divergence": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [12, 24], "threshold": [0.01, 0.015]}
    ...
    "whale_trade": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [8, 16], "threshold": [0.012, 0.018]}
    },
    "funding_settlement": {
        "class": GenericTrendStrategy,
        "param_grid": {"lookback": [6, 12], "threshold": [0.01, 0.015]}
...
# Remove short aliases, keep only unique full strategies
ALL_STRATEGIES = [
    # Core Technical
    ...
    "momentum", "lead_lag", "premium_divergence", "oi_behavior",
    "funding_extreme_reversal", "liquidation_cascade", "cvd_divergence",
    "whale_trade", "funding_settlement"
]

# Verify we have unique strategies
ALL_STRATEGIES = list(dict.fromkeys(ALL_STRATEGIES))
...
class WalkForwardRunner:
    ...
    def generate_report(self) -> str:
        report = []
        report.append("\n" + "="*80)
        report.append(f"Walk-Forward 参数优化报告 - 全部{len(self._results)}策略 (GPU加速)")
        report.append("="*80)
        report.append(f"\n数据: BTCUSDT 真实历史数据")
...
def main():
    print("="*80)
    print(f"Walk-Forward 参数优化 - 全部{len(ALL_STRATEGIES)}策略 (GPU加速)")
    print("="*80)
    print(f"\n策略列表:")
