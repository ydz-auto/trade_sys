...
用法：
    from infrastructure.acceleration import torch, device, is_gpu
...
def _log_device_info():
    ...
    if dtype == "cuda":
        print(f"[GPU] PyTorch GPU: CUDA ({info['name']}, {info['memory_total_gb']:.1f}GB)")
    elif dtype == "mps":
        print(f"[GPU] PyTorch GPU: MPS (Apple Silicon)")
    else:
        print(f"[CPU] PyTorch CPU")
