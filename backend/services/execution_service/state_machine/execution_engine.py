"""
Execution Engine with State Machine - 集成状态机的执行引擎

整合:
- OrderStateMachine: 订单状态机
- OrderReconciliation: 交易所状态同步
- Runtime Governor: 事件优先级
"""
...
class ExecutionEngineWithStateMachine:
    def __init__(
        ...
        self._governor = get_runtime_governor()
    ...
    async def _on_state_change(
        ...
        event = PrioritizedEvent(
            priority=self._get_event_priority(new_state),
        ...
        await self._governor.push_event(event)
    
    def _get_event_priority(self, state: OrderStatus) -> EventPriority:
        if state in (OrderStatus.FILLED, OrderStatus.REJECTED, OrderStatus.FAILED):
            return EventPriority.P0_CRITICAL
        elif state in (OrderStatus.PARTIALLY_FILLED, OrderStatus.CANCELLED):
