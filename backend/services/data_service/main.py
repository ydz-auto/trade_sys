...
async def trigger_collect(collector_name: str):
    ...
    try:
        await collectors[collector_name].collect()
        return {"status": "ok", "collector": collector_name, "message": "Collection triggered"}
    ...
    while True:
        try:
            if "exchange" in collectors:
                prices = await collectors["exchange"].collect()
            ...
            if "etf" in collectors:
                etf_data = await collectors["etf"].collect()
    ...
    while True:
        try:
            if "news" in collectors:
                news_list = await collectors["news"].collect()
            ...
            if "trader" in collectors:
                trader_data = await collectors["trader"].collect()
