"""
LLM Scoring Engine - LLM 增强 + 智能打分 + 中文摘要
...
class ScoringPriority(Enum):
    """打分优先级"""
    P0_FULL = 0  # 完整 LLM（Odaily, ETF, Macro）
    P1_LIGHT = 1  # 轻量 LLM（Twitter, Telegram）
    P2_KEYWORD = 2  # 关键词规则（RSS, Whale）
...
class LLMScoringEngine:
    ...
    async def analyze(self, event: Dict[str, Any]) -> LLMAnalysisResult:
        ...
        Returns:
            LLMAnalysisResult: 包含增强分析 + 多维度打分
        """
        title = event.get("title", "")
        content = event.get("content", "")[:1000]  # 限制长度
