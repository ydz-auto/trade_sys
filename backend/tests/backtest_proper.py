...
class BacktestEngine:
    ...
    def record_capital(self, timestamp: pd.Timestamp):
        """记录资本"""
    ...
    def get_results(self) -> Dict:
        ...
        if self.capital_history and len(self.capital_history) > 1:
            ...
            if len(returns) > 0:
                ...
                sharpe = returns.mean() / (returns.std() + 1e-10) * annual_factor
            else:
                sharpe = 0.0
        ...
        if self.capital_history and len(self.capital_history) > 1:
            ...
            if len(downside_returns) > 0:
                sortino = returns.mean() / (downside_returns.std() + 1e-10) * np.sqrt(365 * 24)
            else:
                sortino = sharpe
        else:
            sortino = 0.0
        ...
        # Calmar
        calmar = -total_profit_pct / (max_drawdown * 100) if max_drawdown != 0 else 0
...
def main():
    print("=" * 120)
    print("PROPER BACKTEST - FIXED ISSUES")
    print("=" * 120)
