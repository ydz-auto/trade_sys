...
async def request_refresh(symbol: str, timeframe: str = "1m", source: str = "api") -> bool:
    from runtime.kernel.event.runtime_bus import get_runtime_bus, MessageType
    bus = get_runtime_bus()
    if bus:
        await bus.publish(
            "refresh_request",
...
async def safe_execute_order(request) -> Any:
    from runtime.kernel.event.router import safe_execute
    return await safe_execute(request)


def get_execution_blocked_error() -> Any:
    from runtime.kernel.event.router import ExecutionBlockedError
