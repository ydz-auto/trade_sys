...
def build_feature_matrix(
    ...
    Args:
        ...
        use_engine: 是否优先使用 FeatureEngine
    ...
    if use_engine:
        try:
            print(f"  尝试使用 FeatureEngine (source='engine')")
            ...
            print(f"  FeatureEngine 成功: {len(df.columns)}列 × {len(df)}行")
            return df
        except Exception as e:
            print(f"  FeatureEngine 不可用: {e}, 回退到传统方式")
...
def _compute_tech_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """使用 AccelerationService 加速计算技术指标"""
    service = get_acceleration_service()
    close = df["close"]
    high = df["high"]
    low = df["low"]
    volume = df["volume"]
    ...
    # 优先尝试使用 GPUExecutor
    if service.is_gpu_available() and len(df) > 1000:
        try:
            ...
            print(f"  技术指标: GPU 加速完成 (via AccelerationService)")
            return df
        except Exception as e:
            print(f"  GPU 计算失败，回退 CPU: {e}")

    # 使用 CPUExecutor（如果有多个独立计算任务可并行）
    # 这里因为是连续的滚动窗口计算，暂不使用并行，直接 CPU 串行计算
    for period in [7, 14, 21]:
        delta = close.diff()
