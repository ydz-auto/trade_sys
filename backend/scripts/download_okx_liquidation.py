#!/usr/bin/env python3
"""
OKX Liquidation 下载脚本
通过 REST API 下载历史强平数据并转换为 Parquet 格式
"""
...
BASE_URL = "https://www.okx.com/api/v5/public/liquidation-orders"
...
def download_liquidation(
    ...
    save_base = data_root / "crypto" / "okx" / "liquidation"
    save_base.mkdir(parents=True, exist_ok=True)
    
    for symbol in symbols:
        print(f"\n{'='*60}")
        print(f"下载 {symbol} Liquidation 数据")
        print(f"{'='*60}")
        ...
        while current_end > start_date:
            try:
                ...
                all_data.extend(liq_data)
                
                oldest_ts = int(liq_data[-1]["ts"])
        ...
        if all_data:
            ...
            print(f"保存 {len(df)} 条记录到 {parquet_path}")
        else:
            print("未获取到数据")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="下载OKX Liquidation数据")
    ...
    print("=" * 60)
    print("OKX Liquidation 数据下载")
    print(f"交易对: {args.symbols}")
    print(f"年份: {args.years}")
    print("=" * 60)
    
    download_liquidation(args.symbols, args.years)
