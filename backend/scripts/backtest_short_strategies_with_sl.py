...
logger = get_logger("short_strategy_backtest_with_sl")
...
class ShortStrategyBacktesterWithSL:
    ...
    def run_strategy_with_sl(
        ...
        for i in range(100, len(df)):
            ...
            if triggered:
                ...
                liquidation = ret_leveraged <= -stop_loss_pct * self.leverage
    ...
    def run_all(self, df: pd.DataFrame) -> Dict:
        """运行所有优化策略"""
        logger.info("Running optimized short strategy backtest with stop loss...")
        ...
        for strategy_key, (detector, config) in detectors.items():
            ...
            trades = self.run_strategy_with_sl(strategy_key, df, detector, config)
    ...
    def print_report(self):
        """打印回测报告"""
        print("\n" + "=" * 140)
        print("📊 BTC 做空/双向策略深度回测报告（带止损版本）")
        print("=" * 140)
        print(f"初始资金: ${self.initial_capital:,.0f} | 杠杆: {self.leverage}x")
        print()
        
        print(f"{'策略':<30} | {'方向':<6} | {'样本':<6} | {'胜率':<8} | {'平均收益':<12} | {'最佳':<10} | {'最差':<10} | {'止损':<6} | {'止盈':<6} | {'时间退':<6} | {'爆仓率':<8}")
        print("-" * 140)
        ...
        for key, r in sorted_results:
            ...
            print(f"{r['name']:<30} | {r['direction']:<6} | {r['events']:<6} | {win_rate_str:<8} | {avg_ret_str:>10} | {best_str:>8} | {worst_str:>8} | {r['stop_loss_count']:<6} | {r['take_profit_count']:<6} | {r['time_exit_count']:<6} | {liq_rate_str}")
        
        print()
        print("=" * 140)
        print("🏆 TOP 3 做空/双向策略（带止损）")
        print("=" * 140)
        ...
        for i, (key, r) in enumerate(sorted_results[:3], 1):
            ...
            print(f"     爆仓率: {r['liquidation_rate']*100:.1f}%")
    ...
    def save_results(self):
        """保存结果"""
        output_path = backend_path / "data_lake/research/short_strategy_backtest_with_sl_results.json"
        ...
        output_data = {
            "backtest_info": {
                ...
                "note": "With real stop loss and take profit logic",
            },
...
def main():
    print("🚀 BTC 做空策略深度回测 - 带止损版本")
    print()
    
    tester = ShortStrategyBacktesterWithSL(initial_capital=10000, leverage=50)
