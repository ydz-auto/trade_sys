...
def main():
    print("="*80)
    print("测试 ret_5_positive_reversal 策略 @ BTCUSDT")
    print("="*80)
    ...
    pipeline = AlphaPipeline(
        ...
        skip_stability=True,
        output_dir="reports/alpha/test_ret5_positive",
        exchange="binance",
        exclude_sources=["oi", "liquidation"]
    )
    
    result = pipeline.run(["ret_5_positive_reversal"])
