...
class BusMessage:
    ...
    topic: str
    payload: Union[BaseEvent, Dict[str, Any]]
...
class RuntimeBus:
    ...
    def __init__(self):
        ...
        from domain.trading_mode import get_trading_mode_manager  # TODO: bus should not depend on domain, inject mode via constructor
    ...
    async def publish(
        ...
        if message_type in (MessageType.EVENT, MessageType.BROADCAST):
            if not isinstance(payload, BaseEvent):
                raise TypeError(
                    f"RuntimeBus.publish() with {message_type.value} requires BaseEvent, "
                    f"got {type(payload).__name__}. "
                    f"Use publish_event(event) for events."
                )
        full_topic = self._get_namespaced_topic(topic) if use_namespace else topic
        ...
        if self._journal is not None and isinstance(payload, BaseEvent):
            try:
                asyncio.create_task(self._journal.append(payload))
...
async def publish_event(event: BaseEvent, **kwargs) -> str:
    bus = get_runtime_bus()
