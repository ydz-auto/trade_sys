...
class RuntimeDependencyGraph:
    ...
    def _setup_default_dependencies(self) -> None:
        self._nodes[RuntimeType.MARKET] = DependencyNode(
            runtime_type=RuntimeType.MARKET,
            priority=100,
        )
        
        self._nodes[RuntimeType.INGESTION] = DependencyNode(
            runtime_type=RuntimeType.INGESTION,
            dependencies={RuntimeType.MARKET},
            priority=90,
        )
        
        self._nodes[RuntimeType.FEATURE] = DependencyNode(
            runtime_type=RuntimeType.FEATURE,
            dependencies={RuntimeType.INGESTION},
            priority=80,
        )
        
        self._nodes[RuntimeType.BEHAVIOUR] = DependencyNode(
            runtime_type=RuntimeType.BEHAVIOUR,
            dependencies={RuntimeType.FEATURE},
            priority=70,
        )
        
        self._nodes[RuntimeType.SIGNAL] = DependencyNode(
            runtime_type=RuntimeType.SIGNAL,
            dependencies={RuntimeType.FEATURE, RuntimeType.BEHAVIOUR},
            priority=60,
        )
        
        self._nodes[RuntimeType.EXECUTION] = DependencyNode(
            runtime_type=RuntimeType.EXECUTION,
            dependencies={RuntimeType.SIGNAL},
            priority=50,
        )
        
        self._nodes[RuntimeType.PORTFOLIO] = DependencyNode(
            runtime_type=RuntimeType.PORTFOLIO,
            dependencies={RuntimeType.EXECUTION},
            priority=40,
        )
        
        self._nodes[RuntimeType.RISK] = DependencyNode(
            runtime_type=RuntimeType.RISK,
            dependencies={RuntimeType.PORTFOLIO},
            priority=30,
        )
        
        self._nodes[RuntimeType.PROJECTION] = DependencyNode(
            runtime_type=RuntimeType.PROJECTION,
            dependencies={RuntimeType.SIGNAL, RuntimeType.EXECUTION, RuntimeType.PORTFOLIO, RuntimeType.RISK},
            priority=20,
        )
        
        self._nodes[RuntimeType.REPLAY] = DependencyNode(
            runtime_type=RuntimeType.REPLAY,
            priority=100,
        )
        
        self._nodes[RuntimeType.NARRATIVE] = DependencyNode(
        ...
        self._nodes[RuntimeType.MONITORING] = DependencyNode(
            runtime_type=RuntimeType.MONITORING,
            priority=10,
            optional=True,
        )
    ...
    def can_start(self, runtime_type: RuntimeType, started: Set[RuntimeType]) -> Tuple[bool, List[RuntimeType]]:
        if runtime_type not in self._nodes:
            return True, []
