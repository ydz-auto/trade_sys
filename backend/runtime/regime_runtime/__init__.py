...
class RegimeRuntime(BaseRuntime):
    ...
    def __init__(self, config: Optional[RegimeRuntimeConfig] = None):
        ...
        self.strategy_registry: Dict[MarketRegime, List[str]] = {
            MarketRegime.HIGH_VOLATILITY: [
                "breakout", "volatility_expansion", "momentum_ignition",
            ],
            MarketRegime.LOW_VOLATILITY: [
                "funding_exhaustion_trap", "bb_compression_breakout",
                "imbalance_pressure",
            ],
            MarketRegime.TRENDING: [
                "trend_following", "momentum_ignition", "breakout",
                "aggressive_flow",
            ],
            MarketRegime.RANGING: [
                "funding_exhaustion_trap", "imbalance_pressure",
                "liquidity_vacuum",
            ],
            MarketRegime.LIQUIDATION_CASCADE: [
                "panic_reversal", "oi_flush", "long_liquidation_bounce",
                "short_squeeze",
            ],
            MarketRegime.NARRATIVE_BURST: [
                "momentum_ignition", "trend_following", "breakout",
            ],
            MarketRegime.LIQUIDITY_DRAIN: [
                "liquidity_vacuum", "imbalance_pressure",
                "sweep_detection",
            ],
    ...
    def should_enable_strategy(self, strategy_id: str) -> bool:
        ...
        if base_id in self.current_regime.active_strategies:
            return True
        return strategy_id in self.current_regime.active_strategies

    def apply_to_orchestrator(self, orchestrator: Any) -> List[str]:
        active = self.get_active_strategies()
        changed: List[str] = []

        for strategy_id, strategy in orchestrator._strategies.items():
            should_be_active = self.should_enable_strategy(strategy_id)
            is_active = strategy.is_enabled

            if should_be_active and not is_active:
                strategy.enable()
                changed.append(strategy_id)
                logger.info(f"Regime {self.current_regime.regime.value}: enabled {strategy_id}")
            elif not should_be_active and is_active:
                strategy.disable()
                changed.append(strategy_id)
                logger.info(f"Regime {self.current_regime.regime.value}: disabled {strategy_id}")

        return changed

    def get_regime_weights(self) -> Dict[str, float]:
        from engines.compute.signal.confluence_engine import REGIME_WEIGHTS
        regime_value = self.current_regime.regime.value
        return REGIME_WEIGHTS.get(regime_value, {})
    ...
    async def on_event(self, event: Any) -> None:
        ...
        if event_type == "market_data_update":
            ...
            await self.update_market_data(
                ...
                liquidation_data=data.get("liquidation_data"),
    ...
    async def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            "regime_state": {
                ...
                "confidence": self.current_regime.confidence,
                "since": self.current_regime.since.isoformat(),
                "duration_seconds": self.current_regime.duration_seconds,
    ...
    async def recover(self, checkpoint: Any = None) -> None:
        ...
        regime_value = regime_data.get("regime", MarketRegime.UNKNOWN.value)
        since = _utcnow()
        if "since" in regime_data:
            since = datetime.fromisoformat(regime_data["since"])
        self.current_regime = RegimeState(
            regime=MarketRegime(regime_value),
            confidence=regime_data.get("confidence", 0.0),
            since=since,
            duration_seconds=regime_data.get("duration_seconds", 0.0),
            features=regime_data.get("features", {}),
            active_strategies=regime_data.get("active_strategies", []),
        )
    ...
    async def get_state(self) -> Dict[str, Any]:
        return {
            "regime_state": self.current_regime.regime.value,
