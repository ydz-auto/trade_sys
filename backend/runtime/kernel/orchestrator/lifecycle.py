...
class RuntimeLifecycle:
        ...
        try:
            ...
            if hasattr(info.instance, 'start'):
                ...
                if task.done():
                    ...
                    info.metadata["lifecycle_task"] = task
                    self._attach_runtime_task_callback(runtime_id, task)

            self._registry.update_state(runtime_id, RuntimeState.RUNNING)
            self._record_transition(runtime_id, from_state, RuntimeState.RUNNING, True)
    ...
    async def stop(self, runtime_id: str) -> bool:
        ...
        try:
            ...
            task = info.metadata.get("lifecycle_task")
            if isinstance(task, asyncio.Task) and not task.done():
                try:
                    await asyncio.wait_for(task, timeout=10.0)
                except asyncio.TimeoutError:
                    logger.warning(f"Runtime {runtime_id} did not stop in time; cancelling task")
                    task.cancel()
                    try:
                        await task
                    except asyncio.CancelledError:
                        pass
            info.metadata.pop("lifecycle_task", None)
    ...
    def _attach_runtime_task_callback(self, runtime_id: str, task: asyncio.Task) -> None:
        def _on_done(done_task: asyncio.Task) -> None:
            info = self._registry.get(runtime_id)
            if not info or info.state in {RuntimeState.STOPPING, RuntimeState.STOPPED}:
                return
            ...
            if exc:
                self._registry.update_state(runtime_id, RuntimeState.FAILED, str(exc))
                logger.error(f"Runtime task failed for {runtime_id}: {exc}")
            elif info.state == RuntimeState.RUNNING:
                self._registry.update_state(runtime_id, RuntimeState.STOPPED)
