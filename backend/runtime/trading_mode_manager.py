...
logger = logging.getLogger("trading_mode.manager")
...
class TradingModeManager:
    ...
    def _get_adapter(self, adapter_name: str):
        if self._adapters and adapter_name in self._adapters:
            return self._adapters[adapter_name]
        if adapter_name == "paper":
            from engines.adapters.exchange.paper_trading_adapter import PaperTradingAdapter
            return PaperTradingAdapter()
        elif adapter_name == "mock":
            from engines.adapters.exchange.mock_adapter import MockAdapter
            return MockAdapter()
        elif adapter_name == "binance_futures":
            from engines.adapters.exchange.binance_futures_adapter import BinanceFuturesAdapter
            return BinanceFuturesAdapter()
        elif adapter_name == "okx":
            from engines.adapters.exchange.okx_adapter import OKXAdapter
            return OKXAdapter()
        return None
    ...
    async def _connect_adapter(self) -> None:
        ...
        if self._mode == TradingMode.BACKTEST:
            adapter = self._get_adapter("mock")
            if adapter and not hasattr(adapter, '_exchange'):
                adapter._exchange = exchange
            self._adapter = adapter
        elif self._mode == TradingMode.PAPER:
            adapter = self._get_adapter("paper")
            if adapter and not hasattr(adapter, '_exchange'):
                adapter._exchange = exchange
            self._adapter = adapter
        elif self._mode == TradingMode.LIVE:
            if self._exchange == "binance":
                self._adapter = self._get_adapter("binance_futures")
            else:
                self._adapter = self._get_adapter("okx")
