...
logger = get_logger("runtime.event_namespace")
...
class EventNamespace:
    ...
    def topic(
        ...
        mode_prefix = self._get_mode_prefix(mode)
        ...
        if full_topic not in self._topic_registry:
            self._topic_registry[full_topic] = EventTopic(
                ...
                domain=domain.value,
                event=event.value,
                mode=(mode or self._mode_manager.mode).value,
            )
    ...
    def redis_channel(self, domain: EventDomain, event: EventType) -> str:
        return self.topic(domain, event)

    def ws_channel(self, domain: EventDomain, event: EventType) -> str:
        return f"ws.{self.topic(domain, event)}"

    def kafka_topic(self, domain: EventDomain, event: EventType) -> str:
        return self.topic(domain, event).replace(".", "_")
...
def get_event_namespace() -> EventNamespace:
