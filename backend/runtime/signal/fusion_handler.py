...
from domain.event.kernel_event.event_buffer import EventBuffer
...
class FusionHandler:
    ...
    def __init__(
        ...
        signal_cooldown_seconds: int = 10,
    ):
    ...
    def generate_signals(self, price_change: float = 0.02) -> List[Signal]:
        ...
        signals = self.engine.process(events=valid_events, price_change=price_change)
        if not signals:
            return []
