...
def main():
    print("="*80)
    print("手动测试 parabolic_runup 做空策略")
    print("="*80)
    ...
    for symbol in symbols:
        ...
        try:
            ...
            feat = fm["parabolic_ret_zscore"].values
            
            print(f"\nparabolic_ret_zscore 分布:")
            ...
            for percentile in [85, 90, 95, 97]:
                ...
                for holding_bars in [5, 10, 20]:
                    # 手动回测：当 parabolic_ret_zscore > threshold 时做空
                    n = len(close)
                    fee = 0.0005 * 2
                    ...
                    if len(valid_idx) >= 30:
                        ...
                        if pf > best_pf and win_rate > 0.5 and pf > 1.0:
                            best_pf = pf
                            best_result = {
            ...
            if best_result:
                ...
                print(f"\n没有找到满足条件 (PF>1.0, WR>0.5, trades>=30) 的参数组合")
