"""
FastAPI Server - 简化版，不依赖复杂的 collectors
"""
...
@app.get("/api/v1/risk")
async def get_risk():
    return {"total": 0.35, "level": "low", "components": {"volatility": 0.3, "flow": 0.4, "sentiment": 0.35, "macro": 0.35}}

@app.get("/api/v1/signal")
async def get_signal():
    return {"action": "HOLD", "confidence": 0.5, "riskLevel": "MEDIUM", "reason": "等待更多信号"}

@app.get("/api/v1/positions")
async def get_positions():
    return [{"symbol": "BTC", "side": "LONG", "size": 0.5, "pnl": 2.5}]

@app.get("/api/v1/weights/versions")
async def get_weight_versions():
    return [{"version": "v1.0", "createdAt": "2026-01-01T00:00:00Z", "factors": {"trend": 0.3, "flow": 0.25, "sentiment": 0.25, "macro": 0.2}}]
