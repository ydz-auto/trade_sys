...
class FeatureSchemaRegistry:
    ...
    def _register_default_schemas(self):
        ...
        self._register(FeatureSchema("aggressive_buy_volume", FeatureCategory.TRADE, "主动买量", "float", requires_lookback=False))
        self._register(FeatureSchema("aggressive_sell_volume", FeatureCategory.TRADE, "主动卖量", "float", requires_lookback=False))
        self._register(FeatureSchema("total_volume", FeatureCategory.TRADE, "总成交量", "float", requires_lookback=False))
        self._register(FeatureSchema("total_value", FeatureCategory.TRADE, "总成交额", "float", requires_lookback=False))
        self._register(FeatureSchema("num_trades", FeatureCategory.TRADE, "成交笔数", "int", requires_lookback=False))
        self._register(FeatureSchema("trade_velocity", FeatureCategory.TRADE, "成交速度", "float", requires_lookback=False))
        self._register(FeatureSchema("avg_trade_size", FeatureCategory.TRADE, "平均单笔成交量", "float", requires_lookback=False))
        self._register(FeatureSchema("max_trade_size", FeatureCategory.TRADE, "最大单笔成交量", "float", requires_lookback=False))
        self._register(FeatureSchema("large_trade_ratio", FeatureCategory.TRADE, "大单占比", "float", requires_lookback=False))
        self._register(FeatureSchema("large_trade_volume", FeatureCategory.TRADE, "大单成交量", "float", requires_lookback=False))
        self._register(FeatureSchema("sweep_buy_score", FeatureCategory.TRADE, "扫单买入分数", "float", requires_lookback=False))
        self._register(FeatureSchema("sweep_sell_score", FeatureCategory.TRADE, "扫单卖出分数", "float", requires_lookback=False))
        self._register(FeatureSchema("liquidity_vacuum", FeatureCategory.TRADE, "流动性真空", "float", requires_lookback=False))
        self._register(FeatureSchema("trade_imbalance", FeatureCategory.TRADE, "买卖失衡", "float", requires_lookback=False))
        self._register(FeatureSchema("buy_sell_ratio", FeatureCategory.TRADE, "买卖比", "float", requires_lookback=False))
        self._register(FeatureSchema("taker_buy_ratio", FeatureCategory.TRADE, "主动买入占比", "float", requires_lookback=False))
        self._register(FeatureSchema("ofi", FeatureCategory.TRADE, "订单流不平衡", "float", requires_lookback=False))
        
        # OI 替代特征 - 压力代理 (TRADE category)
        self._register(FeatureSchema("trade_pressure_score", FeatureCategory.TRADE, "交易压力评分", "float", True, requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("long_pressure_score", FeatureCategory.TRADE, "多头压力评分", "float", True, requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("short_pressure_score", FeatureCategory.TRADE, "空头压力评分", "float", True, requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("squeeze_pressure_score", FeatureCategory.TRADE, "逼空压力评分", "float", requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("flush_pressure_score", FeatureCategory.TRADE, "多杀多压力评分", "float", requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("cvd_delta", FeatureCategory.TRADE, "CVD变化量", "float", requires_lookback=False))
        self._register(FeatureSchema("cvd_zscore", FeatureCategory.TRADE, "CVD Z-score", "float", True, requires_lookback=True, lookback_window=240, available_after_periods=1))
        self._register(FeatureSchema("volume_zscore", FeatureCategory.TRADE, "成交量Z-score", "float", True, requires_lookback=True, lookback_window=240, available_after_periods=1))
